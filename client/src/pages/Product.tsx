    ...
    <div className="container mx-auto px-4 py-8">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-16">
        ...
        <div className="space-y-4">
          <div className="aspect-square bg-white/5 border border-border p-8 flex items-center justify-center relative overflow-hidden group">
            <img 
              src={product.image} 
              alt={product.name} 
          ...
          <div className="grid grid-cols-4 gap-4">
            {[product.image, product.image, product.image, product.image].map((img, i) => (
              <div key={i} className="aspect-square bg-white/5 border border-border p-2 cursor-pointer hover:border-primary transition-colors">
        ...
        <div className="flex flex-col h-full">
          ...
          <h1 className="text-3xl md:text-5xl font-bold font-heading italic uppercase text-white mb-4 leading-tight">
            {product.name}
          </h1>
      ...
      <Tabs defaultValue="specs" className="w-full">
        <TabsList className="w-full justify-start bg-transparent border-b border-border p-0 h-auto rounded-none gap-8">
          <TabsTrigger 
            value="specs" 
            className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:text-primary data-[state=active]:bg-transparent bg-transparent px-0 py-4 font-bold uppercase tracking-wider text-muted-foreground hover:text-white transition-colors text-lg"
          >
            Specifications
          </TabsTrigger>
          <TabsTrigger 
        ...
        <TabsContent value="specs" className="pt-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <div className="bg-card border border-border p-8 max-w-2xl">
            <h3 className="text-xl font-bold font-heading uppercase text-white mb-6">Technical Specifications</h3>
            <div className="grid grid-cols-1 gap-4">
              ...
              <div className="grid grid-cols-2 py-3 border-b border-border/50">
                ...
                <span className="text-white font-mono text-right">{product.sku}</span>
              </div>
            </div>
          </div>
        </TabsContent>
        ...
        <TabsContent value="fitment" className="pt-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
           <div className="bg-card border border-border p-8 max-w-2xl">
            <h3 className="text-xl font-bold font-heading uppercase text-white mb-6">Vehicle Compatibility</h3>
